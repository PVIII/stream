...
#ifndef LIBOUTPUT_VIEW_TAKE_UNTIL_HPP_
#define LIBOUTPUT_VIEW_TAKE_UNTIL_HPP_
...
requires View<V> class take_until_view
    : public view_interface<take_until_view<V, Pred>>
{
  private:
    class __iterator;
    class __sentinel;
  ...
  public:
    take_until_view() = default;

    constexpr take_until_view(V base, Pred pred)
...
template<View V, class Pred> class take_until_view<V, Pred>::__iterator
{
  private:
    iterator_t<V>    current_{};
    take_until_view* parent_               = nullptr;
    bool             predicate_been_false_ = false;
    friend __sentinel;

    class output_proxy
    {
        take_until_view<V, Pred>::__iterator& parent_;
        iterator_t<V>&                        iterator_;

      public:
        output_proxy(take_until_view<V, Pred>::__iterator& p, iterator_t<V>& i)
  ...
  public:
    ...
    constexpr __iterator(take_until_view& parent, iterator_t<V> current)
...
template<Range V, class Pred>
requires View<V> class take_until_view<V, Pred>::__sentinel
{
    sentinel_t<V> end_;
  ...
  public:
    __sentinel() = default;
    explicit constexpr __sentinel(take_until_view& parent)
...
take_until_view(R&&, Pred)->take_until_view<all_view<R>, Pred>;

struct __take_until_fn
{
    template<Range R, class Pred>
    requires ViewableRange<R> constexpr auto operator()(R&&  rng,
                                                        Pred pred) const
    {
        return take_until_view{std::forward<R>(rng), std::move(pred)};
    }
...
{
inline constexpr std::experimental::ranges::ext::output::__take_until_fn
    take_until{};
} // namespace output_view

#endif /* LIBOUTPUT_VIEW_TAKE_UNTIL_HPP_ */
