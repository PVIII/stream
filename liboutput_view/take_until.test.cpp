...
SCENARIO("Assignments.")
{
    ...
    {
        array a{0, 0, 0};
        auto  v = output_view::take_until(a, [](auto v) { return v == 2; });
        ...
        WHEN("[1, 2, 3] is copied.")
        {
            ...
            THEN("[1, 2] is stored.")
            {
                REQUIRE_THAT(a, Equals(array{1, 2, 0}));
            }
        }
        ...
        {
            ext::copy(array{2}, v);
            THEN("[2] is stored.") { REQUIRE_THAT(a, Equals(array{2, 0, 0})); }
        }
        WHEN("[2, 3, 4] is copied.")
        {
            ext::copy(array{2, 3, 4}, v);
            THEN("[2] is stored.") { REQUIRE(equal(a, array{2, 0, 0})); }
        }

        GIVEN("An additional adaptor writing until 3.")
        {
            auto v2 = output_view::take_until(v, [](auto i) { return i == 3; });

            WHEN("[1, 2, 3] is copied.")
            {
                ext::copy(array{1, 2, 3}, v2);
                THEN("[1, 2] is stored.") { REQUIRE(equal(a, array{1, 2, 0})); }
            }
            WHEN("[1, 3, 2] is copied.")
            {
                ext::copy(array{1, 3, 2}, v2);
                THEN("[1, 3] is stored.") { REQUIRE(equal(a, array{1, 3, 0})); }
            }
        }
    }
}
